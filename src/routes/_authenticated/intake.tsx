...
export const Route = createFileRoute("/_authenticated/intake")({
  head: () => ({
    meta: [{ title: "Pet Intake Form — JaxStay" }],
  }),
...
function IntakeForm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!b) {
        setPageError(
          `Booking not found or you do not have permission to open it. Booking ID: ${bookingId}`,
        );
        setLoading(false);
        return;
      }
      ...
      const canView =
        user.id === loadedBooking.owner_id || user.id === loadedBooking.sitter_id;
      ...
      if (existing) {
        ...
        setForm({
          ...
          emergency_care_authorized:
            existing.emergency_care_authorized ?? false,
          ...
          cleaning_supplies_location:
            existing.cleaning_supplies_location ?? "",
        ...
        <section className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
          ...
          <div className="mt-6 rounded-3xl border border-border bg-card p-6 shadow-soft">
            <h1 className="font-display text-2xl font-700">
              Intake form could not open
            </h1>
            <p className="mt-3 whitespace-pre-wrap text-sm text-muted-foreground">
              {pageError}
            </p>
            <p className="mt-3 text-xs text-muted-foreground">
              URL booking ID: {bookingId || "missing"}
            </p>
          </div>
        </section>
      </SiteLayout>
  ...
  return (
    <SiteLayout>
      <section className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        ...
        <h1 className="mt-4 font-display text-3xl font-700 sm:text-4xl">
          Pet intake form
        </h1>

        <p className="mt-1 text-sm text-muted-foreground">
          Give your sitter everything they need to keep your pet safe and happy.
        </p>

        {readOnly && (
          <div className="mt-4 rounded-2xl border border-border bg-muted/40 p-4 text-sm text-muted-foreground">
            You are viewing this as the sitter. Only the pet owner can edit and
            submit this form.
          </div>
        )}
        ...
        <form onSubmit={submit} className="mt-8 space-y-8">
          <Section title="Pet profile">
            <Grid>
              <Field label="Pet name" required>
                <input
                  ...
                  disabled={readOnly}
                  value={form.pet_name}
                  onChange={(e) => upd("pet_name", e.target.value)}
                  className={inp}
                />
              </Field>
              ...
              <Field label="Weight (lbs)">
                <input
                  ...
                  onChange={(e) => upd("weight_lbs", e.target.value)}
                  className={inp}
                />
              </Field>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <Check
                label="Rabies vaccination current"
                checked={form.rabies_current}
                disabled={readOnly}
                onChange={(v) => upd("rabies_current", v)}
              />
              <Check
                label="DHPP vaccination current"
                checked={form.dhpp_current}
                disabled={readOnly}
                onChange={(v) => upd("dhpp_current", v)}
              />
            </div>
          </Section>
          ...
          <Section title="Medical">
            <Grid>
              <Field label="Vet clinic name">
                <input
                  disabled={readOnly}
                  value={form.vet_name}
                  onChange={(e) => upd("vet_name", e.target.value)}
                  className={inp}
                />
              </Field>
              ...
              <Field label="Vet phone">
                <input
                  ...
                  onChange={(e) => upd("vet_phone", e.target.value)}
                  className={inp}
                />
              </Field>
            </Grid>
            ...
            <div className="mt-3">
              <Check
                label="I authorize the sitter to seek emergency veterinary care"
                checked={form.emergency_care_authorized}
                disabled={readOnly}
                onChange={(v) => upd("emergency_care_authorized", v)}
              />
            </div>
          </Section>
          ...
          <Section title="Logistics">
            <Grid>
              <Field label="Emergency contact name">
                <input
                  ...
                  onChange={(e) => upd("emergency_contact_name", e.target.value)}
                  className={inp}
                />
              </Field>
              ...
              <Field label="Emergency contact phone">
                <input
                  ...
                  onChange={(e) =>
                    upd("emergency_contact_phone", e.target.value)
                  }
                  className={inp}
                />
              </Field>
            ...
            <Field label="Update frequency preference">
              <select
                ...
                onChange={(e) => upd("update_frequency", e.target.value)}
                className={inp}
              >
            ...
            <Field label="Cleaning supplies / waste disposal location">
              <input
                disabled={readOnly}
                value={form.cleaning_supplies_location}
                onChange={(e) =>
                  upd("cleaning_supplies_location", e.target.value)
                }
                className={inp}
              />
            </Field>
          ...
          <Section title="Daily care">
            <Field label="Feeding schedule">
              <textarea
                ...
                onChange={(e) => upd("feeding_schedule", e.target.value)}
                className={inp}
              />
            </Field>

            <Field label="Walk routine">
              <textarea
                disabled={readOnly}
                rows={3}
                value={form.walk_routine}
                onChange={(e) => upd("walk_routine", e.target.value)}
                className={inp}
              />
            </Field>
            ...
            <Field label="Trigger notes">
              <textarea
                ...
                onChange={(e) => upd("trigger_notes", e.target.value)}
                className={inp}
              />
            </Field>

            <Field label="Treat restrictions / allergies">
              <input
                disabled={readOnly}
                value={form.treat_restrictions}
                onChange={(e) => upd("treat_restrictions", e.target.value)}
                className={inp}
              />
            </Field>
          </Section>
          ...
          <Section title="Behavior & safety">
            <Field label="Cameras disclosure">
              <textarea
                disabled={readOnly}
                rows={2}
                value={form.cameras_disclosure}
                onChange={(e) => upd("cameras_disclosure", e.target.value)}
                className={inp}
              />
            </Field>

            <Field label="What normal behavior looks like">
              <textarea
                disabled={readOnly}
                rows={2}
                value={form.normal_behavior}
                onChange={(e) => upd("normal_behavior", e.target.value)}
                className={inp}
              />
            </Field>

            <Field label="Red flags requiring immediate contact">
              <textarea
                disabled={readOnly}
                rows={2}
                value={form.red_flags}
                onChange={(e) => upd("red_flags", e.target.value)}
                className={inp}
              />
            </Field>
            ...
            <div className="mt-3">
              <Check
                label="Pet is prone to escaping / bolting"
                checked={form.prone_to_bolting}
                disabled={readOnly}
                onChange={(v) => upd("prone_to_bolting", v)}
              />
            </div>
          </Section>
          ...
          <Section title="Home environment">
            ...
            <Field label="Parking instructions for the sitter">
              <textarea
                disabled={readOnly}
                rows={2}
                value={form.parking_instructions}
                onChange={(e) => upd("parking_instructions", e.target.value)}
                className={inp}
              />
            </Field>
          </Section>
          ...
          {!readOnly && (
            <button
              type="submit"
              disabled={saving}
              className="w-full rounded-full bg-primary px-5 py-3 text-sm font-600 text-primary-foreground shadow-soft transition hover:scale-[1.01] disabled:opacity-50"
            >
              {saving
                ? "Saving…"
                : existingId
...
const inp =
  "mt-1 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary disabled:opacity-70";
